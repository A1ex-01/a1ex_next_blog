...
export default async function Friend() {
    ...
    return list.map(item => <div className="w-full" key={item.id}>
        ...
        <div className="w-full flex flex-wrap">
            {item.children.map(i => (
                <Link key={i.id} href={i.link} className="w-[30vh] flex flex-col justify-between rounded-lg border-[#ccc] border-[1px] border-l-3 border-l-main-color border-solid p-3 mt-4 mr-[3vw] hover:translate-y-[-6px] transition-all hover:shadow-black hover:shadow-sm">
                    <span>{i.name}</span>
                    <hr className="my-1" />
