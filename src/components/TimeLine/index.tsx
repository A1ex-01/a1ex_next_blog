...
export default function TimeLine({ archive }: { archive: ISortByMonth<IPost> }) {
    return <div className="w-full relative border-box">
        <div className="title relative w-full h-20 flex justify-center items-center text-2xl text-main-color">{archive.title}
            <div className="tick absolute top-0 left-[50%] -translate-x-1/2 border-main-color w-3 h-full border-y-4 border-solid"></div>
        </div>
        {archive.list.map(post => <div key={post.id} className="content h-[250px] flex relative items-center">
