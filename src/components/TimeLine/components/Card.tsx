...
export default function TimeLineCard({ post, index }: { post: IPost; index: number }) {
  ...
  return (
    <>
      <div
        className={`time-line w-[80%] ${
          index % 2 === 0 ? 'ml-auto mr-4' : 'mr-auto ml-4'
        } shadow-xl border border-gray-300 border-solid px-6 py-4 relative z-1`}
      >
        <Link
          href={`/post/${post.id}`}
          className="title text-xl text-main-color font-bold hover:opacity-50"
        >
          {post.title}
        </Link>
      ...
      <div
        className={`arrow w-6 h-6 bg-white absolute -z-1`}
        style={transformStyle[index % 2 === 0 ? 0 : 1]}
      ></div>
    </>
  )
}
