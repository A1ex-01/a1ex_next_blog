...
export default function TimeLineCard({ post }: { post: IPost }) {
    return <div className="time-line w-[80%] ml-auto mr-7 relative shadow-xl border border-gray-300 border-solid px-6 py-4">
        <Link href={`/post/${post.id}`} className="title text-xl text-main-color font-bold">{post.title}</Link>
